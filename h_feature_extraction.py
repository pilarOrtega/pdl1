...
def get_probability_images(slidename, classifier, slide_folder, features=30):
    ...
    slide_dz = deepzoom.DeepZoomGenerator(
        slide, tile_size=(224 - 2), overlap=1)
    ...
    image_name = os.path.join(
        outpath, '{}#{}features.npy'.format(slidename, features))
    ...
    for i in range(patches.shape[0]):
        for j in range(patches.shape[1]):
            ...
            patch_name = glob.glob(os.path.join(
                patch_name, '*-{}-{}.jpg'.format(i + 1, j + 1)))
            if not patch_name == []:
                patch = patches[i][j].reshape(
...
def h_feature_extraction(distances, outpath, slide_folder):
    ...
    image_list = Parallel(n_jobs=-3, backend='loky')(delayed(get_probability_images)(
        d[0], d[2], slide_folder, features=n_features) for d in tqdm(distances))

    patch_shape = (3, 3, n_features + 1)
    features = Parallel(n_jobs=-1)(delayed(features_ex)
                                   (i, patch_shape) for i in tqdm(image_list))
